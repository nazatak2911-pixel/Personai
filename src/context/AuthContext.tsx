...
// Helper: load the full registry of all accounts
const loadAccounts = (): User[] => {
  const stored = localStorage.getItem('persona_accounts');
  return stored ? JSON.parse(stored) : [];
};

// Helper: save the full accounts registry
...
export const AuthProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const signup = (name: string, email: string, password: string): { success: boolean; error?: string } => {
    ...
    const newUser: User = { name, email, password, hasCompletedSurvey: false, surveyResults: null, affinityScores: null };
  ...
  const updateName = (newName: string) => {
    ...
    setUser(updatedUser);
    // Update in the global accounts registry
  ...
  return (
    <AuthContext.Provider value={{ user, isLoggedIn: !!user, loginError, login, signup, updateName, logout, completeSurvey }}>
      {children}
