...
export default function OnboardingSurvey() {
  ...
  const { likert: LIKERT_QUESTIONS, mc: MC_QUESTIONS, open: OPEN_QUESTIONS } = t.surveyQuestions;
  ...
  if (isSubmitting) {
    return (
      <div style={{
        ...
        <h2 style={{ color: '#40e0d0', marginTop: '20px' }}>{t.analyzingProfile}</h2>
        <p style={{ color: 'rgba(255,255,255,0.7)', marginTop: '10px' }}>{t.roadmapDesc}</p>
    ...
    <div style={{
      ...
      <div style={{ maxWidth: '800px', width: '100%', margin: 'auto' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '40px' }}>
          <h1 style={{ color: '#40e0d0', fontSize: '2.5rem', textTransform: 'uppercase', marginBottom: '10px' }}>{t.welcomeTrainee}, {user?.name || '...'}</h1>
        ...
        {step === 1 && (
          <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: '24px', padding: '40px', border: '1px solid rgba(64,224,208,0.2)' }}>
            <h2 style={{ color: '#fff', marginBottom: '10px' }}>{t.surveyPart1}</h2>
            <p style={{ color: 'rgba(255,255,255,0.6)', marginBottom: '30px', fontSize: '0.9rem' }}>{t.surveyPart1Desc}</p>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '40px' }}>
              <button onClick={() => setStep(2)} style={btnStyle(true)}>{t.nextPart}</button>
            </div>
          </div>
        )}
        ...
        {step === 2 && (
          <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: '24px', padding: '40px', border: '1px solid rgba(64,224,208,0.2)' }}>
            <h2 style={{ color: '#fff', marginBottom: '10px' }}>{t.surveyPart2}</h2>
            <p style={{ color: 'rgba(255,255,255,0.6)', marginBottom: '30px', fontSize: '0.9rem' }}>{t.surveyPart2Desc}</p>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '40px' }}>
              <button onClick={() => setStep(1)} style={btnStyleOutline()}>{t.back}</button>
              <button onClick={() => setStep(3)} disabled={!isStep2Valid} style={btnStyle(isStep2Valid)}>{t.nextPart}</button>
            </div>
          </div>
        )}
        ...
        {step === 3 && (
          <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: '24px', padding: '40px', border: '1px solid rgba(64,224,208,0.2)' }}>
            <h2 style={{ color: '#fff', marginBottom: '10px' }}>{t.surveyPart3}</h2>
            <p style={{ color: 'rgba(255,255,255,0.6)', marginBottom: '30px', fontSize: '0.9rem' }}>{t.surveyPart3Desc}</p>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '40px' }}>
              <button onClick={() => setStep(2)} style={btnStyleOutline()}>{t.back}</button>
